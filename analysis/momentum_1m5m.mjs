// Intraday MOMENTUM test on AAPL 1m & 5m (the basis for scalping options).
// Momentum = buy new N-bar highs in an up-trend above VWAP (ride continuation).
// Tests long+short with realistic slippage. Uniform 1x sizing (compares signal quality).
...
function momentum(bars, { look = 20, chMult = 2, slip = 0.02, allowShort = true }) {
  const c = bars.map(b => b.c), eF = ema(c, 9), eS = ema(c, 21), a = atr(bars, 14);
  const hi = (i, n) => { let m = -Infinity; for (let k = Math.max(0, i - n); k < i; k++) m = Math.max(m, bars[k].h); return m; };
  const lo = (i, n) => { let m = Infinity; for (let k = Math.max(0, i - n); k < i; k++) m = Math.min(m, bars[k].l); return m; };
  ...
  for (let i = 25; i < bars.length; i++) {
    ...
    if (pos) {
      if (pos.dir > 0) { pos.stop = Math.max(pos.stop, b.h - a[i] * chMult); if (b.l <= pos.stop) cls(pos.stop); }
      else { pos.stop = Math.min(pos.stop, b.l + a[i] * chMult); if (b.h >= pos.stop) cls(pos.stop); }
      if (pos && last) cls(b.c);
    }
    if (!pos && !last && b.min > 35 && b.min < 940) {
      const qty = eq / b.c;
      if (b.c > hi(i, look) && eF[i] > eS[i] && b.c > b.vwap) pos = { entry: b.c, qty, dir: 1, stop: b.c - a[i] * chMult };
      else if (allowShort && b.c < lo(i, look) && eF[i] < eS[i] && b.c < b.vwap) pos = { entry: b.c, qty, dir: -1, stop: b.c + a[i] * chMult };
    }
...
const show = (n, r) => console.log(`  ${n.padEnd(26)} net ${r.net.toFixed(1).padStart(7)}%  trades ${String(r.n).padStart(4)}  win ${r.win.toFixed(0).padStart(3)}%  PF ${Math.min(r.pf, 9).toFixed(2)}  maxDD ${r.maxDD.toFixed(1)}%`);
...
(async () => {
  for (const [iv, rg] of [["5m", "60d"], ["1m", "7d"]]) {
    ...
    console.log(`\n===== ${SYMBOL} ${iv} MOMENTUM — ${bars.length} bars / ${days} days =====`);
    console.log("-- realistic slippage $0.02/side --");
    show("look20 trail2ATR L+S", momentum(bars, { look: 20, chMult: 2, slip: 0.02 }));
    show("look10 trail2ATR L+S", momentum(bars, { look: 10, chMult: 2, slip: 0.02 }));
    show("look20 trail3ATR long-only", momentum(bars, { look: 20, chMult: 3, slip: 0.02, allowShort: false }));
    console.log("-- ZERO slippage (fantasy) --");
    show("look20 trail2ATR L+S", momentum(bars, { look: 20, chMult: 2, slip: 0 }));
  }
})();
