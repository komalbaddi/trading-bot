// =============================================================================
// BTC/USD DAILY TREND BOT — the "better daily" strategy (strat 16).
// EMA 20/50 cross + 200-EMA regime filter + 7xATR trailing stop. Long/flat.
// OOS (2023.5-now): +170%, ~36% drawdown, beat buy&hold on return AND drawdown.
//
// Data: Binance daily (free, current). Execution: Alpaca PAPER crypto (24/7).
// SAFETY: DRY_RUN=true by default = logs only. Set false to place paper orders.
// Run ONCE per day (after the daily bar closes at 00:00 UTC). Cron: "5 0 * * *".
// =============================================================================
...
const STATE = path.join(__dir, "..", "analysis", "btc_daily_state.json");
...
async function barsDaily() {
  const base = "https://data-api.binance.vision/api/v3/klines";
  let start = Date.now() - 400 * 86400000; const out = []; let g = 0;
  while (start < Date.now() && g++ < 5) {
  ...
  return out.filter(b => b.ct < Date.now());   // only fully CLOSED daily bars
}
...
(async () => {
  ...
  const b = await barsDaily(); const c = b.map(x => x.c);
  const eF = ema(c, FAST), eS = ema(c, SLOW), eR = ema(c, REG_LEN), aa = atr(b, ATR_LEN), i = b.length - 1;
  const price = c[i], up = eF[i] > eS[i];
  const regimeOK = !REGIME || price > eR[i];
  const crossUp = eF[i] > eS[i] && eF[i - 1] <= eS[i - 1] && regimeOK;
  const crossDn = eF[i] < eS[i];

  const positions = await api("GET", "/v2/positions");
  const pos = (positions || []).find(p => p.symbol.includes("BTC"));
  const st = loadState();

  console.log(`\n=== BTC DAILY BOT (${DRY_RUN ? "DRY-RUN" : "LIVE PAPER"}) === ${new Date().toISOString()}`);
  console.log(`equity $${equity.toFixed(0)} | BTC $${price.toFixed(0)} | EMA20 ${eF[i].toFixed(0)} EMA50 ${eS[i].toFixed(0)} EMA200 ${eR[i].toFixed(0)} | ${up ? "UPTREND" : "downtrend"}${REGIME ? (price > eR[i] ? " (above 200)" : " (below 200 — filtered)") : ""}`);
  ...
  if (pos && parseFloat(pos.qty) > 0) {
    ...
    s.hi = Math.max(s.hi, b[i].h); s.stop = Math.max(s.stop, s.hi - aa[i] * TRAIL);
    ...
    if (crossUp) {
      ...
      console.log(`BUY: EMA20 crossed above EMA50${REGIME ? " (above 200-EMA)" : ""}. Deploy $${notional} into BTC @ ~$${price.toFixed(0)}.`);
    ...
    } else console.log(`FLAT: no entry (need EMA20 cross above EMA50${REGIME ? " while above 200-EMA" : ""}).`);
  }
  saveState(st);
